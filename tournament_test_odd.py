...
def testRegisterCountDelete():
    ...
    registerPlayer("Curtie Pi")
    c = countPlayers()
    if c != 5:
        raise ValueError(
            "After registering four players, countPlayers should be 5.")
    deletePlayers()
    c = countPlayers()
    if c != 0:
        raise ValueError("After deleting, countPlayers should return zero.")
...
def testStandingsBeforeMatches():
    ...
    if matches1 != 0 or matches2 != 0 or wins1 != 0 or wins2 != 0 or \
       matches3 != 0 or wins3 != 0:
        raise ValueError(
            "Newly registered players should have no matches or wins.")
    if set([name1, name2, name3]) != set(["Melpomene Murray", "Randy Schwartz",\
                                   "Cowboy Curtis"]):
...
def testPairings():
    ...
    registerPlayer("Curtis Mayfield")
    standings = playerStandings()
    [id1, id2, id3, id4, id5] = [row[0] for row in standings]
    reportMatch(id1, id2)
    reportMatch(id3, id4)
    ...
    correct_pairs = set([frozenset([0, id1]), \
                         frozenset([id2, id4]), \
                         frozenset([id3, id5])])
